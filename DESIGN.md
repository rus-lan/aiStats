...
LLM-разметка спорных сегментов — опциональна, за флагом `--llm-phases` (см. §15).
...
LLM-нарратив поверх правил — опционален, за флагом `--llm-narrative` (см. §15). В терминале — топ-N, в HTML — все с раскрытием.
...
- `aistats report [--project <path> | --global] [--tool cc|opencode|all] [--days N | --since <YYYY-MM-DD> --until <YYYY-MM-DD>] [--full] [--json] [--html [path] | --out <path>] [--redact] [--llm-narrative] [--llm-phases [--llm-phases-max <n>]]` — по умолчанию терминал; `--since`/`--until` побеждают `--days`; `--llm-narrative` — опциональный LLM-нарратив поверх ранжированных рекомендаций (нужен `ANTHROPIC_API_KEY`, иначе — уведомление в stderr и деградация к детерминированному выводу); `--llm-phases` — опциональная LLM-доразметка слабых/спорных фазовых блоков только для текущего отчёта (сток фаз никогда не переписывает), лимит блоков — `--llm-phases-max` (по умолчанию 40).
- `aistats export [--project <path> | --global] [--tool cc|opencode|all] [--days N | --since <YYYY-MM-DD> --until <YYYY-MM-DD>] [--redact] [--out <path>] [--pretty]` — строит тот же Report, что и `report`, и пишет его как JSON в `--out` (по умолчанию `~/.aistats/reports/aistats-<scope>-<timestamp>.json`, `--pretty` — с отступами); паттерн копии store в проект — `aistats export --project . --out .aistats/stats.json`.
- `aistats ingest [--session <path>] [--all]` — инкрементальный сбор (зовётся хуками/плагином или вручную).
- `aistats rebuild [--tool cc|opencode|all]` — полный пересбор store из сырья.
- `aistats install [--claude-code | --opencode | --all] [--mcp] [--dry-run]` — вывести/подготовить интеграцию (CC-часть через claude-config); без флага выбора тула = `--all`; `--mcp` дополнительно печатает сниппеты регистрации aistats MCP-сервера для Claude Code (`~/.claude.json`/`.mcp.json`) и Opencode (`opencode.json`).
- `aistats mcp` — запускает MCP-сервер (stdio, JSON-RPC 2.0, newline-delimited JSON); инструменты: `aistats_report`, `aistats_recommendations`, `aistats_projects`.
- `aistats --version` — печатает версию из package.json; `aistats --help` — список команд.

## 15. Реализовано из ранее отложенного

- **MCP-слой** — `aistats mcp`, hand-rolled stdio JSON-RPC 2.0 сервер (инструменты `aistats_report`/`aistats_recommendations`/`aistats_projects`); `aistats install --mcp` печатает конфиг-сниппеты для Claude Code и Opencode.
- **LLM-разметка спорных фаз** — `--llm-phases` на `report`, опционально/за ключом (`ANTHROPIC_API_KEY` или `AISTATS_ANTHROPIC_API_KEY`); без ключа или при ошибке — уведомление в stderr и откат к детерминированной разметке, сток фаз не трогается.
- **LLM-нарратив рекомендаций** — `--llm-narrative` на `report`, опционально/за тем же ключом; без ключа или при ошибке — уведомление в stderr и откат к чистым правилам rule-engine.
- **Дублирование store в проект** — отдельного механизма нет, паттерн закрыт `aistats export --project . --out .aistats/stats.json` (репо-локальный JSON-снапшот отчёта).

Отложенного не осталось.
